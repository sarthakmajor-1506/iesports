...
export interface QueueDoc { id:string; name:string; type:"free"|"wager"|"sponsored"; entryFee:number; bonus:number; sponsorId:string|null; players:QueuePlayer[]; maxPlayers:number; status:"open"|"locked"|"in_progress"|"completed"; createdAt:string; createdBy:string; lobbyId:string|null; messageId:string|null; scheduledTime:string|null;
  // Set when the queue was synthesized from a tournament match (web side).
  // tournamentChannelId routes the lobby embed / announcements to that
  // tournament's private Discord channel instead of the global queue channel.
  tournamentId?:string; tournamentMatchId?:string; tournamentCollection?:string; tournamentChannelId?:string; source?:string; }
...
export async function updateQueue(queueId:string,data:Partial<QueueDoc>|string):Promise<void> { const update = typeof data === "string" ? { status: data as QueueDoc["status"] } : data; await getDb().collection("botQueues").doc(queueId).update(update); }
...
export interface LobbyDoc { id:string; queueId:string; gcLobbyId:string|null; lobbyName:string; password:string; gameMode:string; serverRegion:string; radiant:MatchPlayer[]; dire:MatchPlayer[]; spectators:MatchPlayer[]; status:"waiting"|"active"|"completed"|"cancelled"; dotaMatchId:string|null; winner:"radiant"|"dire"|null; mvp:any|null; duration:string|null; playerStats:any[]|null; createdAt:string; completedAt:string|null; vcRadiantId?:string|null; vcDireId?:string|null;
  // Carried from the tournament-synthesized queue so post-creation messages
  // (teams-assigned / result) land in the tournament channel, not the env one.
