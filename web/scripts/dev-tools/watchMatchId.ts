...
(async () => {
  ...
  for (let i = 0; i < 15; i++) {
    ...
    const cur = `[${new Date().toISOString().slice(11,19)}] lobbyState=${s.lobbyState}(${names[s.lobbyState]||"?"})  members=${s.memberCount}  bot.lobbyMatchId=${s.lobbyMatchId || "—"}  match.dotaMatchId=${m?.dotaMatchId || "—"}  lastLobbyFields=${(s.lastLobbyFields || "").slice(0,80)}`;
