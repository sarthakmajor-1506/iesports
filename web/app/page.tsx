...
interface Tournament {
  ...
  teamSize?: number;
  totalTeams?: number;
  /** Real team count from the teams collection — team mode only. See the API route. */
  teamsBooked?: number;
...
export default function Home() {
  ...
  const [featured, setFeatured] = useState<Partial<Record<GameKey, Tournament>>>({});
  const [completedVal, setCompletedVal] = useState<Tournament | null>(null);
  const [completedDota, setCompletedDota] = useState<Tournament | null>(null);
  const [tournamentLoading, setTournamentLoading] = useState(true);

  // Ticks once a minute so the deadline countdown in the hero stays honest on a
  // tab left open. Seeded at 0 so nothing date-derived renders before the fetch,
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setFeatured({
          dota2: data.dota ?? undefined,
          valorant: data.valorant ?? undefined,
          cs2: data.cs2 ?? undefined,
        });
        setCompletedVal(data.completedValorant ?? null);
        setCompletedDota(data.completedDota ?? null);
      } catch (e) {
        console.error("[Landing] Featured tournament fetch error:", e);
      } finally {
    ...
    <>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* Placeholder for the card, not for the page: it holds the same shape so
           the first fold does not reflow when the tournament arrives. */
        .ie-skel { grid-area: card; background: var(--card); border: 3.5px solid var(--ink); border-radius: 26px; box-shadow: 9px 9px 0 var(--ink); padding: 26px 22px; transform: rotate(-1.1deg); }
        .ie-skel-b { background: rgba(22,19,31,.11); border-radius: 100px; animation: ie-pulse 1.4s ease-in-out infinite; }
        @keyframes ie-pulse { 0%, 100% { opacity: 1; } 50% { opacity: .4; } }
      ...
      <div className="ie-wrap">
        ...
        <section className="ie-hero">
          <div className="ie-hero-grid">
            ...
            {tournamentLoading || !now ? (
              <div className="ie-skel" aria-label="Loading the next tournament">
                <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
                  <div className="ie-skel-b" style={{ width: 130, height: 30 }} />
                  <div className="ie-skel-b" style={{ width: 96, height: 30 }} />
                </div>
                <div className="ie-skel-b" style={{ height: 34, marginTop: 18, width: "78%" }} />
                <div className="ie-skel-b" style={{ height: 16, marginTop: 12, width: "56%" }} />
                <div style={{ display: "flex", gap: 8, marginTop: 20 }}>
                  <div className="ie-skel-b" style={{ height: 30, flex: 2 }} />
                  <div className="ie-skel-b" style={{ height: 30, flex: 1.4 }} />
                  <div className="ie-skel-b" style={{ height: 30, flex: 1 }} />
                </div>
                <div className="ie-skel-b" style={{ height: 17, marginTop: 22 }} />
                <div className="ie-skel-b" style={{ height: 52, marginTop: 20 }} />
              </div>
            ) : nextUp ? (
      ...
      <div className="ie-wrap" style={{ background: "var(--card)", borderTop: "3.5px solid var(--ink)", borderBottom: "3.5px solid var(--ink)" }}>
        ...
        <section className="ie-sec">
          <div className="ie-inner">
            ...
            {tournamentLoading || !now ? (
              <div className="ie-empty" style={{ color: "var(--muted)", fontWeight: 800 }}>Loading…</div>
            ) : alsoOn.length > 0 ? (
