  ...
  if (tLoading) return (
    <>
      <style>{`
        ...
        .dt-sk { background: linear-gradient(90deg,rgba(194,54,22,0.04) 0%,rgba(194,54,22,0.1) 40%,rgba(194,54,22,0.04) 80%); background-size:200% 100%; animation: dt-sk-pulse 1.8s ease-in-out infinite; border-radius:16px; }
      `}</style>
  ...
  return (
    <>
      <style>{`
        ...
        .dt-stat-num.blue { color: #C23616; }
        .dt-stat-num.green { color: #4ade80; }
        .dt-stat-num.light-blue { color: #E25B3E; }
        ...
        .dt-card:hover { transform: translateY(-3px); box-shadow: 0 8px 30px rgba(194,54,22,0.15); border-color: rgba(194,54,22,0.3); }
        ...
        .dt-meta-val.prize { color: #C23616; }
        ...
        .dt-badge-shuffle { font-size: 0.62rem; font-weight: 800; color: #C23616; background: rgba(194,54,22,0.1); border: 1px solid rgba(194,54,22,0.25); border-radius: 100px; padding: 2px 10px; white-space: nowrap; }
        ...
        .dt-reg-btn { padding: 9px 22px; background: #C23616; color: #fff; border: none; border-radius: 100px; font-size: 0.78rem; font-weight: 800; cursor: pointer; transition: all 0.15s; white-space: nowrap; min-width: 130px; text-align: center; }
        .dt-reg-btn:hover { background: #8B1A0E; box-shadow: 0 0 20px rgba(194,54,22,0.4); transform: translateY(-1px); }

        .dt-leaderboard-btn { padding: 9px 22px; background: rgba(226,91,62,0.12); color: #E25B3E; border: 1px solid rgba(226,91,62,0.3); border-radius: 100px; font-size: 0.78rem; font-weight: 800; cursor: pointer; transition: all 0.15s; white-space: nowrap; min-width: 130px; text-align: center; }
        .dt-leaderboard-btn:hover { background: rgba(226,91,62,0.2); box-shadow: 0 0 16px rgba(226,91,62,0.25); transform: translateY(-1px); }
                ...
                {tournaments.filter(t => t.status !== "ended").map((t) => {
                  ...
                  return (
                    <div key={t.id} className={`dt-card${isRegistered ? " registered" : ""}`} style={{ animationDelay: `${0.05 * tournaments.indexOf(t)}s`, position: "relative" }} onClick={() => router.push(`/tournament/${t.id}`)}>
                      ...
                      <div className="dt-card-accent" style={{ background: "#C23616", position: "relative", zIndex: 1 }} />
