  ...
  return (
    <div style={{ marginBottom: 8 }}>
      ...
      {isExpanded && (() => {
        ...
        return (
        <div className="vtd-mc-expanded" style={{ background: "linear-gradient(180deg, #0A0A10 0%, #14141A 30%, #14141A 70%, #0A0A10 100%)", border: "1px solid #2A2A30", borderTop: "none", borderBottomLeftRadius: 10, borderBottomRightRadius: 10, padding: "22px 14px 18px", overflow: "hidden", position: "relative" }}>
...
function ValorantTournamentDetailInner() {
  ...
  if (tLoading) return <TournamentDetailLoader game="valorant" />;
      ...
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        ...
        @media (max-width: 400px) {
          ...
          .vtd-mc-team-tag { font-size: 0.56rem !important; }
          /* Expanded match section */
                                ...
                                <div className="vtd-tier-player" style={{ animationDelay: `${pi * 0.04}s`, ...(isMe ? { background: "rgba(60,203,255,0.10)", boxShadow: "inset 2px 0 0 #3CCBFF", borderRadius: 10 } : {}) }}>
                                  ...
                                  <div className="vtd-tier-player-info">
                                    ...
                                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                                      ...
                                      <RankReportBadge playerUid={p.uid} playerName={p.riotGameName} tournamentId={id} game="valorant" user={user} userName={riotData?.riotGameName || userProfile?.discordUsername || userProfile?.steamName || userProfile?.fullName || "Anonymous"} reports={rankReports} onReportSubmitted={fetchRankReports} nameMap={Object.fromEntries(players.map((pl: any) => [pl.uid || pl.id, (pl.riotGameName && pl.riotTagLine) ? `${pl.riotGameName}#${pl.riotTagLine}` : (pl.riotGameName || "Player")]))} />
                                    </div>
                                  </div>
                                </div>
                              </div>
                  ...
                  <div>
                    ...
                    <div style={{ marginTop: 16 }}>
                      ...
                      <div style={{ padding: "12px 16px", background: "rgba(255,255,255,0.03)", borderRadius: 10, fontSize: "0.78rem", color: "#555550", lineHeight: 1.6, border: "1px solid rgba(255,255,255,0.05)", marginTop: 12 }}>
                        <strong style={{ color: "#8A8880" }}>How it works:</strong> Players are grouped by rank bracket. Each bracket has its own {"\u{1F451}"} MVP — the player with the highest KDA score: (K + 0.2 &times; A) / D. Click any column header to sort. <strong style={{ color: "#8A8880" }}>K</strong> = Kills, <strong style={{ color: "#8A8880" }}>D</strong> = Deaths, <strong style={{ color: "#8A8880" }}>A</strong> = Assists, <strong style={{ color: "#8A8880" }}>ACS</strong> = Avg Combat Score, <strong style={{ color: "#8A8880" }}>ADR</strong> = Avg Damage/Round, <strong style={{ color: "#8A8880" }}>FK</strong> = First Kills, <strong style={{ color: "#8A8880" }}>FD</strong> = First Deaths.
                      </div>
                    </div>
                  </div>
                ) : (() => {
                      ...
                      const playerRankByUid: Record<string, { riotRank: string; riotTier: number; baseRank: string; riotAvatar: string }> = {};
                      players.forEach((p: any) => {
                        const uid = p.uid || p.id;
                        if (uid) {
                          const displayRank = p.iesportsRank || p.riotRank || "";
                          playerRankByUid[uid] = { riotRank: displayRank, riotTier: p.iesportsTier || p.riotTier || 0, baseRank: displayRank.split(" ")[0], riotAvatar: p.riotAvatar || "" };
                        }
                      });
                          ...
                          return (
                            <div key={rankKey} className="vtd-tier-col" style={{ minWidth: 130, maxWidth: 180 }}>
                              ...
                              <div onClick={() => player.uid && router.push(`/player/${player.uid}?tab=valorant`)} style={{ cursor: player.uid ? "pointer" : "default", padding: "12px 10px", display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                                ...
                                <span style={{ fontSize: "0.6rem", fontWeight: 700, color: rankColors[pRank?.baseRank || ""] || "#555550", padding: "1px 6px", borderRadius: 4, background: `${(rankColors[pRank?.baseRank || ""] || "#555550")}15` }}>{pRank?.riotRank || "—"}</span>
                              </div>
                            </div>
                          );
                        })}
                          ...
                          <div style={{ overflowX: "auto" }}>
                            <table className="vtd-standings-table">
                              <thead><tr>
                                ...
                                <th style={thStyle("kd", "#3CCBFF")} onClick={() => setLbSort("kd")}>KDA{sortArrow("kd")}</th>
