  ...
  if (authLoading || tLoading) return (
    <div style={{ minHeight: "100vh", background: "#0A0A0C", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "system-ui,sans-serif" }}>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
        <div style={{ width: 32, height: 32, border: "3px solid #2A2A30", borderTopColor: "#ff4655", borderRadius: "50%", animation: "vtspin 0.8s linear infinite" }} />
        <span style={{ color: "#555550", fontSize: "0.84rem" }}>Loading...</span>
      </div>
    ...
    <>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        .vtd-page { min-height: 100vh; background: #0A0A0C; font-family: var(--font-geist-sans), system-ui, sans-serif; color: #F0EEEA; }
        .vtd-hero { position: relative; height: 200px; background: linear-gradient(135deg, #ff4655 0%, #1a0008 100%); overflow: hidden; }
        .vtd-hero-overlay { position: absolute; inset: 0; background: linear-gradient(to bottom, rgba(10,10,12,0) 0%, rgba(10,10,12,0.6) 70%, rgba(10,10,12,1) 100%); }
        ...
        .vtd-hero-title { font-size: 1.6rem; font-weight: 900; color: #F0EEEA; }
        .vtd-hero-meta { display: flex; gap: 14px; margin-top: 6px; flex-wrap: wrap; }
        .vtd-hero-chip { font-size: 0.72rem; font-weight: 700; padding: 3px 10px; border-radius: 100px; background: rgba(255,255,255,0.08); border: 1px solid #2A2A30; color: #8A8880; white-space: nowrap; }
        .vtd-hero-chip.accent { color: #ff4655; border-color: rgba(255,70,85,0.3); background: rgba(255,70,85,0.1); }
        .vtd-content { max-width: 1100px; margin: 0 auto; padding: 0 30px 60px; }
        .vtd-tabs { display: flex; gap: 0; border-bottom: 1px solid #2A2A30; margin-bottom: 24px; overflow-x: auto; }
        .vtd-tab { padding: 10px 18px; font-size: 0.84rem; font-weight: 600; color: #8A8880; background: none; border: none; cursor: pointer; font-family: inherit; border-bottom: 2px solid transparent; margin-bottom: -1px; transition: all 0.15s; white-space: nowrap; display: flex; align-items: center; gap: 6px; }
        .vtd-tab:hover { color: #ccc; }
        .vtd-tab.active { color: #ff4655; border-bottom-color: #ff4655; font-weight: 800; }
        .vtd-card { background: #121215; border: 1px solid #2A2A30; border-radius: 14px; padding: 20px 24px; margin-bottom: 16px; }
        .vtd-card-label { display: block; font-size: 0.62rem; font-weight: 800; letter-spacing: 0.12em; text-transform: uppercase; color: #555550; margin-bottom: 14px; }
        .vtd-stat-row { display: flex; gap: 12px; flex-wrap: wrap; margin-bottom: 20px; }
        .vtd-stat { flex: 1; min-width: 120px; background: #18181C; border: 1px solid #2A2A30; border-radius: 10px; padding: 12px 16px; text-align: center; }
        .vtd-stat-val { font-size: 1.1rem; font-weight: 800; color: #F0EEEA; }
        .vtd-stat-val.accent { color: #ff4655; }
        .vtd-stat-lbl { font-size: 0.62rem; color: #555550; font-weight: 600; text-transform: uppercase; letter-spacing: 0.08em; margin-top: 2px; }
        .vtd-reg-bar { display: flex; align-items: center; justify-content: space-between; gap: 16px; margin-bottom: 24px; padding: 14px 20px; background: #121215; border: 1px solid #2A2A30; border-radius: 14px; flex-wrap: wrap; }
        .vtd-reg-info { display: flex; flex-direction: column; gap: 2px; }
        .vtd-reg-slots { font-size: 1rem; font-weight: 800; color: #F0EEEA; }
        .vtd-reg-countdown { font-size: 0.78rem; color: #8A8880; }
        ...
        .vtd-reg-done { padding: 10px 24px; background: rgba(22,163,74,0.12); color: #4ade80; border: 1px solid rgba(34,197,94,0.3); border-radius: 100px; font-size: 0.86rem; font-weight: 700; }
        ...
        .vtd-tl-date { font-size: 0.82rem; color: #8A8880; }
        .vtd-tl-badge { font-size: 0.58rem; font-weight: 800; padding: 2px 7px; border-radius: 100px; margin-left: 8px; }
        .vtd-rule { display: flex; gap: 10px; padding: 8px 0; border-bottom: 1px solid #1e1e22; }
        .vtd-rule:last-child { border-bottom: none; }
        .vtd-rule-num { font-size: 0.78rem; font-weight: 800; color: #ff4655; min-width: 22px; }
        .vtd-rule-text { font-size: 0.84rem; color: #8A8880; line-height: 1.5; }
        .vtd-empty { text-align: center; padding: 60px 20px; }
        .vtd-empty-icon { font-size: 42px; margin-bottom: 8px; display: block; }
        .vtd-empty-title { font-size: 0.95rem; font-weight: 700; color: #8A8880; margin-bottom: 4px; display: block; }
        .vtd-empty-sub { font-size: 0.82rem; color: #555550; display: block; margin-top: 4px; }

        .vtd-players-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; }
        .vtd-player-box { background: #121215; border: 1px solid #2A2A30; border-radius: 14px; padding: 20px; display: flex; align-items: center; gap: 16px; transition: box-shadow 0.15s; }
        .vtd-player-box:hover { box-shadow: 0 2px 12px rgba(0,0,0,0.3); }
        .vtd-player-avatar-lg { width: 56px; height: 56px; border-radius: 10px; object-fit: cover; background: #1a1a1f; flex-shrink: 0; }
        .vtd-player-avatar-init { width: 56px; height: 56px; border-radius: 10px; background: #1a1a1f; display: flex; align-items: center; justify-content: center; font-size: 20px; font-weight: 800; color: #555550; flex-shrink: 0; }
        ...
        .vtd-player-rank-lg { font-size: 0.74rem; color: #8A8880; margin-top: 2px; }
        ...
        .vtd-team-box { background: #121215; border: 1px solid #2A2A30; border-radius: 16px; padding: 24px; position: relative; transition: box-shadow 0.15s; }
        .vtd-team-box:hover { box-shadow: 0 2px 16px rgba(0,0,0,0.3); }
        .vtd-team-box-num { position: absolute; top: 12px; right: 14px; font-size: 0.62rem; font-weight: 800; color: #ff4655; background: rgba(255,70,85,0.1); border: 1px solid rgba(255,70,85,0.25); padding: 2px 8px; border-radius: 100px; }
        ...
        .vtd-team-box-avg { font-size: 0.68rem; color: #555550; margin-top: 2px; }
        ...
        .vtd-team-box-member-avatar { width: 34px; height: 34px; border-radius: 8px; object-fit: cover; background: #1a1a1f; flex-shrink: 0; }
        ...
        .vtd-team-box-member-name { font-size: 0.84rem; font-weight: 600; color: #e0e0da; }
        ...
        .vtd-team-box-footer { display: flex; justify-content: space-between; align-items: center; margin-top: 16px; padding-top: 12px; border-top: 1px solid #1e1e22; font-size: 0.72rem; color: #555550; }
        .vtd-team-edit-btn { padding: 5px 14px; background: #18181C; border: 1px solid #2A2A30; border-radius: 100px; font-size: 0.72rem; font-weight: 700; cursor: pointer; color: #8A8880; font-family: inherit; transition: all 0.15s; }
        .vtd-team-edit-btn:hover { border-color: #ff4655; color: #ff4655; }
        .vtd-team-edit-input { width: 100%; padding: 8px 12px; border: 1.5px solid #ff4655; border-radius: 8px; font-size: 0.84rem; outline: none; font-family: inherit; background: #0A0A0C; color: #F0EEEA; }
        ...
        .vtd-team-edit-cancel { padding: 6px 16px; background: #18181C; color: #8A8880; border: 1px solid #2A2A30; border-radius: 100px; font-size: 0.72rem; font-weight: 700; cursor: pointer; font-family: inherit; }

        .vtd-standings-table { width: 100%; border-collapse: collapse; }
        .vtd-standings-table th { font-size: 0.62rem; font-weight: 800; text-transform: uppercase; letter-spacing: 0.08em; color: #555550; padding: 8px 12px; text-align: left; border-bottom: 1px solid #2A2A30; }
        .vtd-standings-table td { font-size: 0.84rem; padding: 10px 12px; border-bottom: 1px solid #1e1e22; color: #e0e0da; }
        ...
        .vtd-section-header.group { color: #555550; border-color: #2A2A30; }
        .vtd-section-header.bracket { color: #f59e0b; border-color: rgba(253,230,138,0.3); }

        .vtd-match-day-header { font-size: 0.74rem; font-weight: 800; text-transform: uppercase; letter-spacing: 0.1em; color: #555550; margin: 20px 0 10px; padding-bottom: 6px; border-bottom: 1px solid #2A2A30; display: flex; align-items: center; gap: 8px; }
        ...
        .vtd-mc { display: flex; align-items: center; background: #121215; border: 1px solid #2A2A30; border-radius: 10px; margin-bottom: 8px; overflow: hidden; transition: box-shadow 0.15s; min-height: 64px; }
        .vtd-mc:hover { box-shadow: 0 2px 12px rgba(0,0,0,0.3); }
        .vtd-mc-index { width: 40px; flex-shrink: 0; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 2px; padding: 8px 0; border-right: 1px solid #1e1e22; }
        .vtd-mc-index-num { font-size: 0.68rem; font-weight: 800; color: #555550; }
        .vtd-mc-index-fmt { font-size: 0.54rem; font-weight: 800; color: #ff4655; background: rgba(255,70,85,0.1); padding: 1px 5px; border-radius: 4px; }
        ...
        .vtd-mc-team-name { font-size: 0.82rem; font-weight: 700; color: #F0EEEA; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .vtd-mc-avatars { display: flex; gap: 0; margin-top: 3px; }
        .vtd-mc-avatars img, .vtd-mc-avatars .vtd-mc-av-init { width: 20px; height: 20px; border-radius: 50%; border: 1.5px solid #121215; margin-left: -4px; object-fit: cover; }
        ...
        .vtd-mc-score-box .s { min-width: 20px; text-align: center; color: #F0EEEA; }
        ...
        @media (max-width: 800px) { .vtd-hero-content { padding: 0 16px; padding-bottom: 16px; } .vtd-hero-title { font-size: 1.2rem; } .vtd-content { padding: 0 16px 40px; } .vtd-overview-grid { grid-template-columns: 1fr; } .vtd-stat-row { gap: 8px; } .vtd-stat { min-width: 80px; padding: 10px; } .vtd-mc-avatars { display: none; } .vtd-mc-team-name { font-size: 0.76rem; } }
        @media (max-width: 600px) { .vtd-players-grid { grid-template-columns: 1fr; } .vtd-teams-grid { grid-template-columns: 1fr; } .vtd-mc-team { padding: 8px 10px; gap: 8px; } .vtd-mc-team-logo { width: 30px; height: 30px; font-size: 9px; } }
      `}</style>
      ...
      <div className="vtd-page">
          ...
          <div className="vtd-hero-content">
            <div>
              ...
              <div className="vtd-hero-meta">
                ...
                {tournament.prizePool && tournament.prizePool !== "0" && <span className="vtd-hero-chip accent">{tournament.prizePool.startsWith("₹") ? tournament.prizePool : `₹${tournament.prizePool}`}</span>}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="vtd-content">
          ...
          <div className="vtd-tabs">
            {TABS.map(t => (
              <button key={t.key} className={`vtd-tab${activeTab === t.key ? " active" : ""}`} onClick={() => setActiveTab(t.key)}>
                ...
                {t.key === "players" && <span style={{ fontSize: "0.68rem", color: "#555550", fontWeight: 600 }}>({players.length})</span>}
                {t.key === "teams" && teams.length > 0 && <span style={{ fontSize: "0.68rem", color: "#555550", fontWeight: 600 }}>({teams.length})</span>}
              </button>
            ))}
          </div>
            ...
            <div className="vtd-overview-grid">
              <div>
                <div className="vtd-stat-row">
                  <div className="vtd-stat"><div className="vtd-stat-val">{tournament.format === "shuffle" ? "SHUFFLE" : tournament.format?.toUpperCase()}</div><div className="vtd-stat-lbl">Format</div></div>
                  <div className="vtd-stat"><div className="vtd-stat-val">BO2</div><div className="vtd-stat-lbl">Match Format</div></div>
                  <div className="vtd-stat"><div className="vtd-stat-val">5</div><div className="vtd-stat-lbl">Team Size</div></div>
                  <div className="vtd-stat"><div className="vtd-stat-val accent">{tournament.prizePool || "TBD"}</div><div className="vtd-stat-lbl">Prize Pool</div></div>
                </div>
                <div className="vtd-card"><span className="vtd-card-label">About</span><p style={{ fontSize: "0.88rem", color: "#8A8880", lineHeight: 1.7 }}>{tournament.desc}</p></div>
                <div className="vtd-card"><span className="vtd-card-label">Rules</span>{(tournament.rules || []).map((rule: string, i: number) => (<div key={i} className="vtd-rule"><span className="vtd-rule-num">{i + 1}.</span><span className="vtd-rule-text">{rule}</span></div>))}</div>
              </div>
              <div>
          ...
          {activeTab === "players" && (
            <div><div className="vtd-card"><span className="vtd-card-label">Registered Players ({players.length})</span>{players.length === 0 ? (<div className="vtd-empty"><span className="vtd-empty-icon">👤</span><span className="vtd-empty-title">No players registered yet</span><span className="vtd-empty-sub">Be the first to register!</span></div>) : (<div className="vtd-players-grid">{players.map((p: any) => (<Link key={p.uid} href={`/player/${p.uid}`} style={{ textDecoration: "none", color: "inherit", display: "block" }}><div className="vtd-player-box">{p.riotAvatar ? <img className="vtd-player-avatar-lg" src={p.riotAvatar} alt={p.riotGameName} /> : <div className="vtd-player-avatar-init">{(p.riotGameName || "?")[0].toUpperCase()}</div>}<div className="vtd-player-info"><div className="vtd-player-name-lg">{p.riotGameName}<span className="tag">#{p.riotTagLine}</span></div><div className="vtd-player-rank-lg">{p.riotRank || "Unranked"}</div><span className="vtd-player-skill-lg" style={{ background: p.skillLevel >= 4 ? "rgba(146,64,14,0.15)" : p.skillLevel >= 3 ? "rgba(99,102,241,0.12)" : "#1a1a1f", color: p.skillLevel >= 4 ? "#fbbf24" : p.skillLevel >= 3 ? "#818cf8" : "#8A8880", border: `1px solid ${p.skillLevel >= 4 ? "rgba(251,191,36,0.3)" : p.skillLevel >= 3 ? "rgba(99,102,241,0.3)" : "#2A2A30"}` }}>Skill {p.skillLevel || 1}</span></div></div></Link>))}</div>)}</div></div>
          )}

          {activeTab === "teams" && (
            <div>{teams.length === 0 ? (<div className="vtd-empty"><span className="vtd-empty-icon">👥</span><span className="vtd-empty-title">Teams not generated yet</span><span className="vtd-empty-sub">Teams will be shuffled after registration closes.</span></div>) : (<div className="vtd-teams-grid">{teams.map((team: any) => { const isMyTeam = userTeam?.id === team.id; const canEdit = isMyTeam && !team.teamNameSet; const isEditing = editingTeamId === team.id; return (<div key={team.id} className="vtd-team-box"><span className="vtd-team-box-num">#{team.teamIndex}</span><div className="vtd-team-box-header"><div className="vtd-team-logo" style={{ position: "relative", cursor: (isMyTeam && !team.teamLogoSet && !logoUploading) ? "pointer" : "default" }} onClick={() => { if (isMyTeam && !team.teamLogoSet && !logoUploading) logoInputRef.current?.click(); }}>{team.teamLogo ? <img src={team.teamLogo} alt={team.teamName} /> : getTeamInitials(team.teamName)}{isMyTeam && !team.teamLogoSet && !logoUploading && (<div style={{ position: "absolute", inset: 0, borderRadius: 12, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", opacity: 0, transition: "opacity 0.15s" }} onMouseEnter={e => (e.currentTarget.style.opacity = "1")} onMouseLeave={e => (e.currentTarget.style.opacity = "0")}><span style={{ color: "#fff", fontSize: 18 }}>📷</span></div>)}{logoUploading && (<div style={{ position: "absolute", inset: 0, borderRadius: 12, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center" }}><div style={{ width: 18, height: 18, border: "2px solid #555", borderTopColor: "#ff4655", borderRadius: "50%", animation: "vtspin 0.8s linear infinite" }} /></div>)}</div><input ref={logoInputRef} type="file" accept="image/*" style={{ display: "none" }} onChange={e => { const f = e.target.files?.[0]; if (f) handleLogoUpload(team.id, f); }} /><div style={{ flex: 1, minWidth: 0 }}>{isEditing ? (<div><input className="vtd-team-edit-input" value={newTeamName} onChange={e => setNewTeamName(e.target.value)} placeholder="Enter team name" maxLength={24} autoFocus onKeyDown={e => { if (e.key === "Enter") handleUpdateTeamName(team.id); }} />{teamNameError && <div style={{ fontSize: "0.68rem", color: "#f87171", marginTop: 4 }}>{teamNameError}</div>}<div className="vtd-team-edit-actions"><button className="vtd-team-edit-save" onClick={() => handleUpdateTeamName(team.id)} disabled={teamNameLoading}>{teamNameLoading ? "Saving..." : "Save"}</button><button className="vtd-team-edit-cancel" onClick={() => { setEditingTeamId(null); setTeamNameError(""); }}>Cancel</button></div></div>) : (<><div className="vtd-team-box-name">{team.teamName}</div><div className="vtd-team-box-avg">Avg Skill: {team.avgSkillLevel}</div>{logoError && isMyTeam && <div style={{ fontSize: "0.62rem", color: "#f87171", marginTop: 4 }}>{logoError}</div>}</>)}</div></div><div className="vtd-team-box-members">{(team.members || []).map((m: any, i: number) => (<div key={m.uid || i} className="vtd-team-box-member">{m.riotAvatar ? <img src={m.riotAvatar} alt={m.riotGameName} className="vtd-team-box-member-avatar" /> : <div className="vtd-team-box-member-init">{(m.riotGameName || "?")[0]}</div>}<div style={{ flex: 1, minWidth: 0 }}><div className="vtd-team-box-member-name">{m.riotGameName}</div><div className="vtd-team-box-member-rank">{m.riotRank}</div></div><span className="vtd-team-box-member-skill">Skill {m.skillLevel}</span></div>))}</div><div className="vtd-team-box-footer"><span>{team.members?.length || 0} players</span><div style={{ display: "flex", gap: 6, alignItems: "center" }}>{canEdit && !isEditing && <button className="vtd-team-edit-btn" onClick={() => { setEditingTeamId(team.id); setNewTeamName(team.teamName); setTeamNameError(""); }}>✏️ Set Team Name</button>}{isMyTeam && !team.teamLogoSet && !logoUploading && <button className="vtd-team-edit-btn" onClick={() => logoInputRef.current?.click()}>📷 Set Logo</button>}{team.teamNameSet && <span style={{ fontSize: "0.62rem", color: "#4ade80" }}>✓ Name set</span>}{team.teamLogoSet && <span style={{ fontSize: "0.62rem", color: "#4ade80" }}>✓ Logo set</span>}</div></div></div>); })}</div>)}</div>
          )}

          {activeTab === "standings" && (
            <div className="vtd-card"><span className="vtd-card-label">Group Stage Standings</span>{standings.length === 0 ? (<div className="vtd-empty"><span className="vtd-empty-icon">🏆</span><span className="vtd-empty-title">No standings yet</span><span className="vtd-empty-sub">Standings will appear once matches are played.</span></div>) : (<div style={{ overflowX: "auto" }}><table className="vtd-standings-table"><thead><tr><th>#</th><th>Team</th><th>P</th><th>W</th><th>D</th><th>L</th><th style={{ color: "#4ade80" }}>MW</th><th style={{ color: "#f87171" }}>ML</th><th style={{ color: "#ff4655" }}>Pts</th><th>BH</th></tr></thead><tbody>{standings.map((s: any, i: number) => (<tr key={s.id}><td style={{ fontWeight: 800, color: i < 6 ? "#ff4655" : "#555550" }}>{i + 1}</td><td style={{ fontWeight: 700 }}>{s.teamName}</td><td>{s.played || 0}</td><td>{s.wins || 0}</td><td>{s.draws || 0}</td><td>{s.losses || 0}</td><td style={{ color: "#4ade80" }}>{s.mapsWon || 0}</td><td style={{ color: "#f87171" }}>{s.mapsLost || 0}</td><td style={{ fontWeight: 800, color: "#ff4655" }}>{s.points || 0}</td><td style={{ color: "#555550" }}>{s.buchholz || 0}</td></tr>))}</tbody></table></div>)}</div>
          )}

          {activeTab === "matches" && (
            <div>{matches.length === 0 ? (<div className="vtd-empty"><span className="vtd-empty-icon">⚔️</span><span className="vtd-empty-title">No matches scheduled</span><span className="vtd-empty-sub">Matches will appear once pairings are generated.</span></div>) : (<>{groupMatches.length > 0 && (<div><div className="vtd-section-header group">Group Stage Fixtures</div>{(() => { const days = [...new Set(groupMatches.map((m: any) => m.matchDay))].sort((a: number, b: number) => a - b); return days.map((day: number) => (<div key={day}><div className="vtd-match-day-header"><span className="day-num">Round {day}</span><span>· {groupMatches.filter((m: any) => m.matchDay === day).length} matches</span></div>{groupMatches.filter((m: any) => m.matchDay === day).map((m: any) => (<MatchCard key={m.id} m={m} teamMembers={teamMembers} teamLogoMap={teamLogoMap} expandedMatch={expandedMatch} setExpandedMatch={setExpandedMatch} tournamentId={id} isBracket={false} />))}</div>)); })()}</div>)}{bracketMatches.length > 0 && (<div style={{ marginTop: groupMatches.length > 0 ? 32 : 0 }}><div className="vtd-section-header bracket">Bracket Fixtures</div>{(() => { const days = [...new Set(bracketMatches.map((m: any) => m.matchDay))].sort((a: number, b: number) => a - b); let bracketRoundNum = 0; return days.map((day: number) => { bracketRoundNum++; const dayMatches = bracketMatches.filter((m: any) => m.matchDay === day); return (<div key={day}><div className="vtd-match-day-header bracket-round"><span className="day-num">Bracket Round {bracketRoundNum}</span><span>· {dayMatches.length} matches</span></div>{dayMatches.map((m: any) => (<MatchCard key={m.id} m={m} teamMembers={teamMembers} teamLogoMap={teamLogoMap} expandedMatch={expandedMatch} setExpandedMatch={setExpandedMatch} tournamentId={id} isBracket={true} />))}</div>); }); })()}</div>)}</>)}</div>
          )}

          {activeTab === "brackets" && <div><DoubleBracket matches={bracketMatches} bracketSize={tournament.bracketSize || 4} standings={standings} /></div>}

          {activeTab === "leaderboard" && (
            <div className="vtd-card"><span className="vtd-card-label">Player Leaderboard — MVP Tracker</span>{leaderboard.length === 0 ? (<div className="vtd-empty"><span className="vtd-empty-icon">📊</span><span className="vtd-empty-title">No stats yet</span><span className="vtd-empty-sub">Player stats will appear once match data is fetched.</span></div>) : (<div style={{ overflowX: "auto" }}><table className="vtd-standings-table"><thead><tr><th>#</th><th>Player</th><th>Agent(s)</th><th>Maps</th><th style={{ color: "#4ade80" }}>K</th><th style={{ color: "#f87171" }}>D</th><th>A</th><th style={{ color: "#ff4655" }}>K/D</th><th>ACS</th><th>HS%</th><th>DMG</th></tr></thead><tbody>{leaderboard.map((p: any, i: number) => { const acs = Math.round((p.totalScore || 0) / Math.max(1, p.totalRoundsPlayed || 1)); return (<tr key={p.id} style={i === 0 ? { background: "rgba(245,158,11,0.08)" } : {}}><td style={{ fontWeight: 800, color: i === 0 ? "#f59e0b" : i < 3 ? "#ff4655" : "#555550" }}>{i === 0 ? "👑" : i + 1}</td><td>{p.uid ? (<Link href={`/player/${p.uid}`} style={{ textDecoration: "none", color: "inherit" }}><div style={{ fontWeight: 700 }}>{p.name}</div><div style={{ fontSize: "0.68rem", color: "#555550" }}>#{p.tag}</div></Link>) : (<><div style={{ fontWeight: 700 }}>{p.name}</div><div style={{ fontSize: "0.68rem", color: "#555550" }}>#{p.tag}</div></>)}</td><td style={{ fontSize: "0.72rem", color: "#8A8880" }}>{(p.agents || []).join(", ")}</td><td>{p.matchesPlayed || 0}</td><td style={{ fontWeight: 700, color: "#4ade80" }}>{p.totalKills || 0}</td><td style={{ color: "#f87171" }}>{p.totalDeaths || 0}</td><td>{p.totalAssists || 0}</td><td style={{ fontWeight: 800, color: (p.kd || 0) >= 1.0 ? "#4ade80" : "#f87171" }}>{p.kd || 0}</td><td style={{ fontWeight: 700 }}>{acs}</td><td>{p.hsPercent || 0}%</td><td style={{ fontSize: "0.78rem", color: "#8A8880" }}>{p.totalDamageDealt || 0}</td></tr>); })}</tbody></table><div style={{ marginTop: 12, padding: "10px 14px", background: "#18181C", borderRadius: 8, fontSize: "0.72rem", color: "#555550", lineHeight: 1.6 }}><strong style={{ color: "#8A8880" }}>How MVP is determined:</strong> Players ranked by Average Combat Score (ACS = total score / rounds played), then K/D ratio as tiebreaker.</div></div>)}</div>
          )}
        </div>
      </div>
